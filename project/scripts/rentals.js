const baseUrl =
  "https://ernesto-quispe.github.io/wdd230/project/data/pricing.json";
...
function updateRentalCards(data) {
  const vehicles = document.querySelectorAll(".vehicle");

  data.rentalPricing.forEach((rental, index) => {
    let vehicle = vehicles[index];

    let img = vehicle.querySelector("img");
    img.src = `images/rentals/${rental.type}/${rental.image}-500.webp`;
    img.alt = rental.Model;

    let title = vehicle.querySelector("h3");
    title.textContent = rental.rentalModel;

    let maxPersons = vehicle.querySelector(".text p");
    maxPersons.textContent = `Max Riders: ${rental.maxPersons}`;

    let pricing = vehicle.querySelectorAll(".pricing p");
    pricing[0].textContent = `Reservations from: $${rental.pricing["reservation"].halfDay}`;
    pricing[1].textContent = `Walk-Ins from: $${rental.pricing["walkIn"].halfDay}`;
  });
}
...
function fillTable(data) {
  ...
  data.rentalPricing.forEach(function (item) {
    ...
    [
      item.rentalModel,
      item.maxPersons,
      item.pricing.reservation.halfDay,
      item.pricing.reservation.fullDay,
      item.pricing.walkIn.halfDay,
      item.pricing.walkIn.fullDay,
